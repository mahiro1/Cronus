...
ACMD(mapmove) {
	...
	if( pc_isdead(sd) ) {
		clif->message(fd, msg_txt(882)); // "You cannot use this command when dead."
		return true;
	}
...
ACMD(go) {
	...
	const struct {
		...
		{ MAP_PRONTERA,    156, 188, 3 }, //  0 = Prontera
	...
	if( pc_isdead(sd) ) {
		clif->message(fd, msg_txt(883)); // "You cannot use this command when dead."
		return true;
	}
